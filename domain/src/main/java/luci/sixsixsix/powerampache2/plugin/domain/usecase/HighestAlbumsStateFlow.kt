...
class HighestAlbumsStateFlow @Inject constructor(private val musicFetcher: MusicFetcher) {
    operator fun invoke(useMock: Boolean = false): StateFlow<List<Album>> =
        if (!useMock)
            musicFetcher.highRatedAlbumsFlow
        else
            MutableStateFlow(AlbumsMock.getRandomAlbumList(11))
